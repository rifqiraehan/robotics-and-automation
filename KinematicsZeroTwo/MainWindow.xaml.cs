...
namespace KinematicsZeroTwo
{
    public partial class MainWindow : Window
    {
        ...
        private double AngleToPulse(int channel, double angleDeg)
        {
            ...
            double angleToConvert = angleDeg;
            double angleRangeDeg = 180.0;

            if (channel == 0)
            {
                angleToConvert = angleDeg + 90.0;
                angleRangeDeg = 180.0;
            }
            ...
            {
                angleToConvert = angleDeg;
                angleRangeDeg = 180.0;
            }

            if (channel == 0 || channel == 2 || channel == 3)
            {
                angleToConvert = Clamp(angleToConvert, 0.0, 180.0);
            }

            double normalized = angleToConvert / angleRangeDeg;
